...
class EditSportsQuiz extends Component {
  ...
  componentDidMount() {
    const request = new Request('http://localhost:8080/sport/');
    fetch(request)
  ...
  handleSubmit(event) {
    ...
    fetch('http://localhost:8080/sport/' + this.state.item.qno, {
        method: 'GET',
      })
      .then(response => response.json())
      .then(data => this.setState({
...
editRecord(event){
  event.preventDefault();
  fetch('http://localhost:8080/sport/' + this.state.select.qno, {
      method: 'PUT',
